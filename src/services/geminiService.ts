...
export const generateQuestions = async (
  ...
  const generateSingleBatch = async (batchNum: number, batchStartIdx: number, batchLetters?: string[], category?: string): Promise<Question[]> => {
    // Take the most recent 100 exclusions to prevent repetition
    const limitedExclusions = excludedAnswers ? excludedAnswers.slice(0, 100) : [];
    const exclusionText = limitedExclusions.length > 0 
      ? `\nمهم جداً (أولوية قصوى): يمنع منعاً باتاً تكرار أي من الإجابات التالية لأنها استخدمت في جولات سابقة: [${limitedExclusions.join("، ")}]. ابحث عن حقائق ومعلومات جديدة ومختلفة تماماً.`
      : "";
...
1. يجب أن تكون الإجابة "صواب" أو "خطأ" فقط.
2. اجعل السؤال عبارة عن معلومة غريبة أو حقيقة مذهلة (مخادعة) تجعل اللاعب يشك في معلوماته.
3. يمنع منعاً باتاً ترك حقل "answer" فارغاً.
` : "";
...
${trueFalseMechanism}
${varietyInstruction}
${isTrueFalseMode ? "مهم جداً: هذا النمط مخصص للحقائق المخادعة والمعاكسة للتوقع تماماً. يجب أن يشعر اللاعب بالصدمة أو الحيرة عند معرفة الإجابة الصحيحة." : ""}
...
4. التنسيق: أرجع البيانات كمصفوفة JSON تحتوي على ${batchNum} كائناً.`;
...
تنبيه هام: حقل "answer" يجب أن يحتوي على الإجابة الصحيحة دائماً.`;
  ...
  try {
    ...
    if (allQuestions.length === 0) {
      throw new Error("فشل توليد الأسئلة من الذكاء الاصطناعي. يرجى التأكد من إعدادات الـ API والمحاولة مرة أخرى.");
    }
