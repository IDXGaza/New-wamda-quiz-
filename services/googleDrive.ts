...
const WEB_CLIENT_ID = '911335724064-2fsqm3qlsciugqe7tri6vk33814uuerq.apps.googleusercontent.com';
...
export const signInToDrive = async (): Promise<{ user: User; accessToken: string }> => {
  if (Capacitor.isNativePlatform()) {
    // APK: use native Google Auth plugin
...
export const uploadBackupToDrive = async (zipBlob: Blob, accessToken: string): Promise<any> => {
  ...
  const metadata = {
    ...
    description: 'نسخة احتياطية لتطبيق ترانيم'
  };

  const boundary = 'traneem_boundary_998877';
