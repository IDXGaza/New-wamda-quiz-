    ...
    <>
      ...
      <aside className={`fixed xl:relative inset-y-0 right-0 w-[85%] sm:w-[400px] bg-white dark:bg-slate-950 border-l border-slate-200 dark:border-slate-800 flex flex-col shadow-2xl xl:shadow-none z-[70] transition-all duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full xl:translate-x-0'}`}>
        ...
        <nav ref={navRef} className="flex-1 overflow-y-auto px-5 pb-10 space-y-3 scroll-container">
          {showBackupReminder && (
            <div className="mb-4 p-4 rounded-[24px] bg-gradient-to-br from-indigo-500 to-indigo-700 text-white shadow-lg animate-in slide-in-from-top-2 duration-300">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 text-right">
                  ...
                  <p className="text-[10px] font-bold opacity-90 leading-relaxed">
                    لقد أضفت أكثر من 5 أناشيد جديدة منذ آخر نسخة احتياطية. نوصيك بحفظ نسخة الآن لتجنب فقدانها.
                  </p>
                  <button 
                    onClick={onOpenBackup}
                    className="mt-3 w-full py-2 bg-white text-indigo-600 rounded-xl text-[11px] font-black hover:bg-white/90 active:scale-95 transition-all"
                  >
